...
% \newenvironment{cs}[3]{\ttfamily{\(S_{cos}(#1, #2) = #3\)}}
...
The primary objective of K-means clustering is to group similar data points in multi-dimensional space. These groups can then be assigned colours. The colours can then be used to draw the data points in any dimensionality, preserving the patters.
...
% }}}
      \subsubsection{nanpa}
% {{{
There are only four words which are related to \textbf{nanpa} (numerals) in the vocabulary of Toki Pona: \tp{wan} used to mean \textit{one}, \tp{tu} used to mean \textit{two}, \tp{luka} used to mean \textit{five}, and \tp{mute}. With the exception of \textbf{mute}, which can be used to denote any large quantity, these words form an independent cluster in the lower left corner of the projection.
...
% {{{
The cluster \tp{soweli} contains words that refer to animals: \tp{akesi}, \tp{kala}, \tp{waso}, \tp{pipi}, \tp{akesi}.
...
The onomatopoeia \tp{mu} is also present in this cluster: \cs{soweli}{mu}{0.54}. Among other words in the cluster, \textbf{pipi} is the least semantically related to \textbf{mu}: \cs{pipi}{mu}{0.29}.

The dictionary entry for \textbf{akesi} contains the now considered outdated \textit{non-cute animal} word sense. The fact that this definition was in use at a time is evident from the semantic similarity between \tp{suwi} and \textbf{akesi}: \cs{akesi}{suwi}{0.27}; compared to the other members of the cluster: \cs{soweli}{suwi}{0.53}, \cs{kala}{suwi}{0.51}, \cs{waso}{suwi}{0.49}, \cs{pipi}{suwi}{0.45}.

The word \tp{mani} has the secondary definition of a \textit{large domenticated animal}. This could be attributed to the themes of hunter-gatherer culture that permeate the language. In the model, only a very weak semantic connection between \textbf{mani} and \textbf{soweli} is present: \cs{mani}{soweli}{0.18}.
% }}}
...
% master plan: pick not the clusters on the model, but what you would expect to
% be a cluster that in reality isn't

% like lawa should be next to sijelo, but isn't because another word sense is
% dominanting it

A small cluster of \tp{kalama} includes words related to sound production and perception: \tp{uta}, \tp{kute}. The earlier discussed \textbf{mu} is also in the close proximity of this cluster.
...
\begin{exe}
  ...
  \glt When I'm listening to music, I want to dance.
\end{exe}
...
% maybe should take shapes instead? like palisa, linja, kiwen, lipu

% luka is away cause it's a number
% lawa and sijelo because ike things
% kute and uta because kalama, also kute means ears
...
\begin{figure}[ht]% {{{
  ...
  \end{tikzpicture}
  \centering
  \caption{sijelo, \(\angle\textdegree\)}
\end{figure}% }}}
...
%       \subsubsection{tenpo}
%
% The words related to \textbf{tenpo} (time) can be divided into the following groups:
%
% \paragraph{nanpa}
%
% The words from the \textbf{nanpa} cluster are often used to quantify time. The numerals are listen in the descending order of magnitude and are added together:
%
% \begin{exe}
%   \ex
...
% \end{figure}% }}}
%
% \paragraph{pini and kama}
%
% In terms of \textbf{tenpo}, the words \tp{pini} and \tp{kama} are antonyms used to refer to the \textit{past} and \textit{future} time respectively. Similar contexts hint at their semantic relatedness: \cs{pini}{kama}{0.47}. The true antonym of \textbf{pini} is \tp{open}
%
% The words \textbf{pini} and \textbf{open} are higly related due to their usage of pre-verbs to mean \textit{finish doing something} and \textit{begin doing something} respectively: \cs{pini}{open}{0.67}.



% , while \textbf{kama} contrasts with \tp{tawa} and \tp{weka}.
%
% However, the words \textbf{kama} and \textbf{tawa} share barely any semantic relatedness: \cs{kama}{tawa}{-0.07}. This is explained by the fact that \textbf{tawa} is characterised by its primary sense of \textit{physical movement} or its use as a preposition \textit{to}. On the other hand, \textbf{kama} is often used as a pre-verb to express a change of state, for example:
%
% \begin{exe}
%   \ex
%   \gll mi sona ala e ni. taso, mi kama sona e ni. \\
%        I know not $do$ this but I become know $do$ this. \\
%   \glt I don't know this, but I'm learning about it.
% \end{exe}
