...
% TODO: this could be extended later with more formal definitons.
...
% {{{
In linguistics, the term \textit{artificial} can denote several types of languages. The subject of this research are languages that have been constructed to be similar or comparable in function to natural languages. ``A constructed language is a language whose phonology, grammar, and vocabulary are consciously devised instead of having developed naturally \parencite{jazayeri}''. In regards to this paper, the term \textit{constructed} was chosen consciously because it follows the convention set by the community and excludes computer programming languages such as Lisp and C++ from the discussion.

Within the community, these languages are often referred to as \textit{conlangs} for the sake of briefness. ``This term was coined by Danish linguist Otto Jespersen when he debuted his own conlang Novial in 1928 \parencite{sanders}''.
...
From the belief in the connection between language creation and the divine arose the notion that languages inherited by humans from gods had become corrupted and now were causing confusion among the scholars and philosophers, impeding scientific progress. In his essay, Wilkins proposed a universal philosopical language to replace Latin. This language was to be unambiguous and to encompass every concept in the universe, to overcome the curse of Babel \parencite[ch. 2, p. 1]{wilkins}. Despite the initial acclaim and the interest it received from the king, the language soon fell into obscurity \parencite[25]{okrent}.

\subsection{Motivation}

\subsection{Classification}

\subsection{Notable languages}
