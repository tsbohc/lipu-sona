...
% TODO: maybe it's better to reformulate the goal to "fill in the gaps in a
% the dictionary to better reflect the way the language is spoken"
...
% TODO idiolects within communities

% Some dialectical variations in the way the language is spoken also exist. While some speakers do not shy away from the more recent and less adopted additions to the vocabulary (\textit{nimi ku suli} and \textit{nimi ku lili}), others wish to stay true to the original design of the language (\textit{pu-rists}) and intentionally limit their effective vocabulary.
...
The term \textit{artificial} can denote several types of languages. The subject of this research are languages that have been constructed to be similar or comparable in function to natural languages. The term \textit{constructed} was chosen because it follows the convention set by the community and in the interest of excluding computer programming languages such as Lisp and C++ from the discussion.

Within the community these languages are often referred to as \textit{conlangs} for the sake of briefness. ``This term was coined by Danish linguist Otto Jespersen when he debuted his own conlang Novial in 1928'' \parencite{punske}.

\subsection{Brief history}

\subsubsection{Ancient times}

The concept of a constructed language was first mentioned in writing by Athenaeus of Naucratis in his \textit{Deipnosophistae} (\textit{circa} \textsc{AD} 230). The languages he presented were not full languages, but languages of rudimentary type known as a \textit{naming language}. These languages were collections of neologisms that could be used to replace existing vocabulary or to refer to things that otherwise had no name. Athenaeus further writes of other people who invented their own words \parencite{punske}.

\subsubsection{Linguistic mysticism}

``The belief in the connection between language creation and divinity shaped many early conlangs. If gods created languages, what would it mean for a human to dabble in this enterprise?'' \parencite{punske}.

From this view arose the notion that languages inherited by humans from gods became corrupted through use and now were causing confusion among the scholars and philosophers. In his essay, Wilkins proposed a universal philosopical language to replace Latin. This language was to be unambiguous and to encompass every concept in the universe \parencite[ch. 2, p. 1]{wilkins}.

\subsection{Reasons for creation}
...
\begin{longtable}{llp{10cm}}
  ...
  \multicolumn{2}{c}{\textbf{Word}} & \textbf{Definition} \\
  \endhead
  a or kin & \textit{particle} & (emphasis, emotion or confirmation) \\
  akesi & \textit{noun} & non-cute animal; reptile, amphibian \\
  ala & \textit{adjective} & no, not, zero \\
  alasa & \textit{verb} & to hunt, forage \\
  \multirow[t]{3}{*}{ale or ali} & \textit{adjective} & all; abundant, countless, bountiful, every, plentiful \\
  ...
  & \textit{number} & five \\
  \multirow[t]{3}{*}{lukin or oko} & \textit{noun} & eye \\
  ...
  sin or namako & \textit{adjective} & new, fresh; additional, another, extra \\
  sina & \textit{noun} & you \\
