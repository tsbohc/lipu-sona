...
\begin{figure}[ht]% {{{
  ...
  \begin{tikzpicture}[scale=0.046]
    \begin{scope}
      \clip (-160, -140) rectangle (190, -30);
      \draw[lightgray, rotate=-40] (-500, -500) grid[xstep=20, ystep=20] (500, 500);
    \end{scope}
    \foreach \Point/\PointLabel/\Colo in {
      (157.36,70.82)/kule/40,
      (129.82,-58.54)/pimeja/64,
    ...
    \draw[draw=black!\Colo,fill=black!\Colo, rotate=-40] \Point circle (2.4) node[above] {\ttfamily\PointLabel};
  \end{tikzpicture}
  \centering
  \caption{kule}
\end{figure}% }}}

The presence of \tp{kasi} among the colours can be attributed to the fact that Toki Pona has only one word for blue and green -- \textbf{laso}. Despite the existence of many languages that also have one word for blue and green, the distribution is evident of the fact that many Toki Pona speakers opt to refer to the color green unambiguously \cs{kasi}{kule}{0.51}.

The word \textbf{pimeja} is considerably separated from the rest of the colours. This is explained by it often appearing near \tp{tenpo}. In turn, \textbf{tenpo pimeja} is a common way of referring to \textit{nighttime}. Among words for colours, this property is unique to \textbf{pimeja}. In fact, it experiences an even split between \textbf{kule} and \textbf{tenpo}: \cs{pimeja}{kule}{0.24}, \cs{pimeja}{tenpo}{0.24}.
