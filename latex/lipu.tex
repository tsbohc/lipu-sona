...
The aim of this research is to bridge the gap between the two disciplines, to use computational linguistics to build a model of a constructed language. This model can then be used to explore the nuances of how humans speak the said language. That is, to gain insight into the semantics of the language.
...
% The last sentence from above excerpt now finds its way into the modern papers on computational linguistics that discuss semantic and linguistics distributions.
...
The position of a word in the semantic space is determined by the values in its word vectors, that is, the context it is most commonly used in. Words that are often used together are placed close to each other. The distance between points is a measurement of semantic similarity between the words they represent.

% In Figure .3, the words \textit{sun} and \textit{mooon} are much nearer than \textit{sun} and \textit{typewriter}. The co
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[>={Classical TikZ Rightarrow[]}]
    ...
    % \node[text width=6cm, color=lightgray] at (5.25,6.5) {NEAREST NEIGHBOURS};
...
% (2.1,3.9)/mun, (2.8,4.8)/lete, (3.4, 6.8)/sewi, 
  ...
  \end{tikzpicture}
  \centering
  \caption{Visualising the example distribution}
\end{figure}
