...
\tikzset{%
  mybackground/.style={%
    ...
    background grid/.style={draw=lightgray, step=1cm, rotate=140},
...
\begin{figure}[ht]% {{{
  ...
  \begin{tikzpicture}[scale=0.046]
    ...
    \tikzset{shift={(160,130)}}
    \draw[draw=black!\Colo,fill=black!\Colo, rotate=-35] \Point circle (2.4) node[above] {\ttfamily\PointLabel};
    \begin{scope}[on background layer]
      \clip ([shift={(-20,-20)}] current bounding box.south west) rectangle ([shift={(20, 20)}] current bounding box.north east);
      \draw[lightgray, rotate=-35] (-500, -500) grid[xstep=20, ystep=20] (500, 500);
    \end{scope}
  \end{tikzpicture}
  \centering
  \caption{kule}
...
The word \textbf{pimeja} is considerably separated from the rest of the colours. This is explained by it often appearing near \tp{tenpo}. In turn, \textbf{tenpo pimeja} is a common way of referring to \textit{nighttime}. Among words for colours, this property is unique to \textbf{pimeja}: \cs{walo}{tenpo}{-0.03}, \cs{loje}{tenpo}{-0.07}, \cs{jelo}{tenpo}{-0.11}, \cs{laso}{tenpo}{-0.06}. Interestingly, \textbf{pimeja} experiences an even semantic split between \textbf{kule} and \textbf{tenpo}: \cs{pimeja}{kule}{0.24}, \cs{pimeja}{tenpo}{0.24}.
...
\begin{figure}[ht]% {{{
  ...
  \begin{tikzpicture}[scale=0.046]
    \begin{scope}[rotate=30]
      ...
      \tikzset{shift={(160,140)}}
      \draw[draw=black!\Colo,fill=black!\Colo] \Point circle (2.4) node[above] {\ttfamily\PointLabel};
    \end{scope}
    \begin{scope}[on background layer]
      \clip ([shift={(-20,-20)}] current bounding box.south west) rectangle ([shift={(20, 20)}] current bounding box.north east);
      \draw[lightgray, rotate=30] (-500, -500) grid[xstep=20, ystep=20] (500, 500);
    \end{scope}
  \end{tikzpicture}
  \centering
  \caption{nanpa}
\end{figure}% }}}
      ...
      There are only three words which can denote \textbf{nanpa} (numerals) in the vocabulary of Toki Pona: \tp{wan} (used to mean \textit{one}), \tp{tu} (used to mean \textit{two}), \tp{luka} (used to mean \textit{five}). These words form an independent cluster in the lower left corner of the projection.

% compare luka with noka and other body parts
